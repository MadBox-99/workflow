...
const DynamicField = ({ label, value, onChange, type = 'text', placeholder, isDynamic, onDynamicChange, availableInputs = [], rows = 3 }) => {
                        ...
                        <div className={`flex items-center gap-2 p-2 rounded border ${
                            ...
                            <span className={`text-sm ${hasActionOutput ? 'text-blue-700 dark:text-blue-300' : 'text-purple-700 dark:text-purple-300'}`}>
                                ...
                                {availableInputs[0].isActionOutput && (
                                    <span className="text-xs ml-1 opacity-75">(use {'{{{input}}}'} or {'{{{input.fieldName}}}'})</span>
                                )}
                            </span>
                        </div>
                    ) : (
...
const extractPaths = (obj, prefix = '', maxDepth = 5) => {
    ...
    if (obj && typeof obj === 'object') {
        if (Array.isArray(obj)) {
            ...
            if (obj.length > 0) {
                paths.push({ path: prefix, type: 'array', preview: `Array[${obj.length}]` });
            ...
            Object.keys(obj).forEach(key => {
                ...
                if (value === null) {
                    ...
                    if (Array.isArray(value)) {
                        paths.push({ path: fullPath, type: 'array', preview: `Array[${value.length}]` });
                        if (value.length > 0) {
                            const itemPaths = extractPaths(value[0], `${fullPath}.0`, maxDepth - 1);
                            paths.push(...itemPaths);
                        }
                    } else {
                        paths.push({ path: fullPath, type: 'object', preview: '{...}' });
...
const ApiCallConfig = ({ config, onChange, nodeId, nodes = [], edges = [] }) => {
    ...
    const updateResponseMapping = (index, field, value) => {
        setResponseMapping(prev => prev.map((item, i) =>
    ...
    const testRequest = async () => {
        ...
        try {
            ...
            if (authToken) {
                parsedHeaders['Authorization'] = authToken.startsWith('Bearer ') ? authToken : `Bearer ${authToken}`;
            }
    ...
    }, [method, url, requestBody, headers, authToken, dynamicFields, responseMapping, outputField, availablePaths]);
        ...
        return (
            <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                {availablePaths.map((item, idx) => (
                    ...
                    >
                        <span className="font-mono text-gray-800 dark:text-gray-200">{item.path}</span>
                        <span className={`text-xs px-1.5 py-0.5 rounded ${
                            item.type === 'string' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                            item.type === 'number' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                            item.type === 'boolean' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                            item.type === 'array' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                            item.type === 'object' ? 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-400' :
                            'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'
                        }`}>
                            {item.type}
                        </span>
    ...
    return (
        <div className="space-y-3">
            ...
            <DynamicField
                ...
                availableInputs={availableInputs.filter(i => i.targetField === 'url' || i.isActionOutput)}
            />
            {!dynamicFields.url && (
            ...
            {(method === 'POST' || method === 'PUT' || method === 'PATCH') && (
                <>
                    <DynamicField
                        ...
                        availableInputs={availableInputs.filter(i => i.targetField === 'requestBody' || i.isActionOutput)}
                    />
            ...
            <DynamicField
                ...
                availableInputs={availableInputs.filter(i => i.targetField === 'headers' || i.isActionOutput)}
            />
            ...
            <DynamicField
                ...
                availableInputs={availableInputs.filter(i => i.targetField === 'authToken' || i.isActionOutput)}
            />
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-3 mt-3">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                    Send a test request to see the response structure and easily select which fields to extract.
                </p>
            ...
            <div className="space-y-3">
                <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                        Select a specific field to pass to the next node. Leave empty to pass the entire response.
                    </p>
                    <div className="relative">
                        <div className="flex gap-2">
                            <input
                                ...
                                onFocus={() => availablePaths.length > 0 && setShowPathDropdown('output')}
                            ...
                            {availablePaths.length > 0 && (
                                <button
                                    type="button"
                                    onClick={() => setShowPathDropdown(showPathDropdown === 'output' ? null : 'output')}
                        ...
                        {showPathDropdown === 'output' && renderPathDropdown(setOutputField, outputField)}
                    </div>
                </div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                Extract multiple fields with custom names for easy access in subsequent nodes.
                            </p>
                        </div>
                        <button
                            type="button"
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {availablePaths
                                    ...
                                    .map((path, idx) => {
                                        ...
                                        const suggestedAlias = lastPart.replace(/[^a-zA-Z0-9]/g, '');
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span className="font-mono">{path.path.length > 20 ? '...' + path.path.slice(-18) : path.path}</span>
                                            </button>
                                        );
                                    })}
                            </div>
                            {availablePaths.filter(p => ['string', 'number', 'boolean'].includes(p.type)).length > 12 && (
                                <p className="text-xs text-gray-400 mt-1">
                                    +{availablePaths.filter(p => ['string', 'number', 'boolean'].includes(p.type)).length - 12} more fields available
                                </p>
                            )}
                        </div>
                    )}
                                ...
                                <div key={index} className="flex gap-2 items-center relative">
                                    <div className="flex-1 relative">
                                        <input
                                            ...
                                            onFocus={() => availablePaths.length > 0 && setShowPathDropdown(index)}
                                        ...
                                        />
                                        {showPathDropdown === index && renderPathDropdown(
                                            (path) => updateResponseMapping(index, 'path', path),
                                            mapping.path
                                        )}
                                    </div>
                                    ...
                                    <input
                                        type="text"
                                        value={mapping.alias}
                                        onChange={(e) => updateResponseMapping(index, 'alias', e.target.value)}
                    ...
                    {responseMapping.length === 0 && availablePaths.length === 0 && (
                        <div className="text-center py-3 border border-dashed border-gray-300 dark:border-gray-600 rounded">
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                Use "Test Request" to see available fields, or click "Add Manual" to type a path.
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded border border-amber-200 dark:border-amber-600">
                        <div className="flex-1">
                            <code className="text-xs font-mono text-amber-800 dark:text-amber-300">{'{{{input}}}'}</code>
                    ...
                    {availablePaths.filter(p => ['string', 'number', 'boolean'].includes(p.type)).slice(0, 3).map((path, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded border border-amber-200 dark:border-amber-600">
                            <div className="flex-1">
                            ...
                            <button
                                type="button"
                                onClick={() => navigator.clipboard.writeText(`{{{input.${path.path}}}}`)}
                                className="p-1 text-amber-600 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded"
                                title="Copy to clipboard"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {responseMapping.filter(m => m.path && m.alias).map((mapping, idx) => (
                        <div key={`mapped-${idx}`} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded border border-purple-200 dark:border-purple-600">
                            <div className="flex-1">
                            ...
                            <button
                                type="button"
                                onClick={() => navigator.clipboard.writeText(`{{{_mapped.${mapping.alias}}}}`)}
            ...
            <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-700">
                <p className="text-xs text-blue-600 dark:text-blue-400">
                    <strong>Tip:</strong> Use the "Test Request" button to see the response structure and easily select fields from the dropdown.
                </p>
            </div>
        </div>
    );
