...
const ConditionConfig = ({ config, onChange, nodeId, nodes, edges }) => {
    ...
    const connectedInputs = useMemo(() => {
        ...
        const inputAEdge = edges.find(
            (e) => e.target === nodeId && e.targetHandle === "input-a",
        );
        const inputBEdge = edges.find(
            (e) => e.target === nodeId && e.targetHandle === "input-b",
        );

        const inputANode = inputAEdge
            ? nodes.find((n) => n.id === inputAEdge.source)
            : null;
        const inputBNode = inputBEdge
            ? nodes.find((n) => n.id === inputBEdge.source)
            : null;
        ...
        <div className="space-y-4">
            ...
            <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
                    The workflow will only continue to connected nodes when this
                    condition is met.
                </p>
            </div>
            ...
            <div className="space-y-3">
                <div>
                    ...
                    <input
                        ...
                        value={defaultValueA}
                        onChange={(e) =>
                            updateConfig({ defaultValueA: e.target.value })
                        }
                ...
                {!isUnaryOperator && (
                    <div>
                        ...
                        <input
                            ...
                            value={defaultValueB}
                            onChange={(e) =>
                                updateConfig({ defaultValueB: e.target.value })
                            }
                            placeholder="Leave empty to require connection"
                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
            ...
            <div className="bg-gray-100 dark:bg-gray-700 rounded p-3 text-sm">
                <div className="flex items-center gap-2">
                    ...
                    ></span>
                    <span className="text-gray-700 dark:text-gray-300">
                        Continues when condition is{" "}
                        <strong>{passWhen.toUpperCase()}</strong>
                    </span>
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                    The output can be connected to multiple nodes - all will
                    receive the result when the condition passes.
                </p>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded p-2">
                <p className="text-xs text-blue-800 dark:text-blue-400">
                    <strong>Tip:</strong> Connect constant or action outputs to
                    inputs A and B. When the condition matches your "pass when"
                    setting, the workflow continues to connected nodes.
                </p>
            </div>
        </div>
    );
